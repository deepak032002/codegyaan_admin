...
import { useCreateCategoryMutation, useGetCategoriesQuery, useGetTagsQuery } from '../../redux/services/apiSlice'
...
const MyEditor = () => {
  ...
  const { handleBlur, handleChange, values, errors, touched, setFieldValue, handleSubmit } = useFormik({
    initialValues: {
      title: '',
      description: '',
      content: '',
        ...
        <div className='md:col-span-9 col-span-12'>
          <Editor
            ...
            init={{
              ...
              plugins: 'image lists emoticons wordcount codesample table fullscreen searchreplace advlist autosave',
          ...
          <span className='text-xs text-red-500'>{touched.content && errors.content ? errors.content : ''}</span>
        </div>
          ...
          <Card color='transparent' shadow={false}>
            <form onSubmit={handleSubmit} className='mb-2'>
              <div className='mb-4 flex flex-col gap-6'>
                <div>
                  <Input size='lg' name='title' value={values.title} onChange={handleChange} onBlur={handleBlur} label='Title' />
                  <span className='text-xs text-red-500'>{touched.title && errors.title ? errors.title : ''}</span>
                </div>
                <div>
                  <Textarea name='description' value={values.description} onChange={handleChange} onBlur={handleBlur} label='Description' />
                  <span className='text-xs text-red-500'>{touched.description && errors.description ? errors.description : ''}</span>
                </div>

                <div>
                  <Select
                    options={category?.data?.categories.map((item) => ({ value: item._id, label: item.name.toUpperCase() }))}
                    ...
                    onBlur={handleBlur}
                  />
                  <span className='text-xs text-red-500'>{touched.category && errors.category ? errors.category : ''}</span>
                </div>
                <div>
                  <Select
                    options={tag?.data?.tags.map((item) => ({ value: item._id, label: item.name.toUpperCase() }))}
                    isMulti
                    placeholder='Tags'
                    onBlur={handleBlur}
                    onChange={(value) =>
                  ...
                  />
                  <span className='text-xs text-red-500'>{touched.tags && errors.tags ? errors.tags : ''}</span>
                </div>
              </div>
